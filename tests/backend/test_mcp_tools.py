...
class TestMCPTools:
    ...
    def test_speak_text_basic(self, mock_elevenlabs, temp_config_dir, mock_subprocess, monkeypatch):
        ...
        mock_elevenlabs["generate"].assert_called_with(text="Hello, World!", voice=None, model="model1")
        
        # Verify temp file cleanup
        temp_files = list(Path("/tmp").glob("*.mp3"))
        assert len(temp_files) == 0
    
    def test_speak_text_with_custom_voice(self, mock_elevenlabs, temp_config_dir, mock_subprocess, monkeypatch):
    ...
    def test_speak_text_with_save_audio(self, mock_elevenlabs, temp_config_dir, mock_subprocess, monkeypatch):
    ...
    def test_error_handling(self, mock_elevenlabs, temp_config_dir, monkeypatch):
        ...
        assert result["success"] is True  # This should still succeed as we're just passing through to the mock 